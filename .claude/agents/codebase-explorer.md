...
- `src/schema/` — Y.Doc schema, field registry, migration system (ydoc.ts, migrations.ts)
...
- `src/components/panels/` — Side panels (sync status, user presence)
- `src/sheets/` — Google Sheets sync (SheetsAdapter, sheetsMapper, BaseValueStore, sheetsClient, oauth)
- `src/collab/` — Real-time collaboration (Yjs/CRDT, awareness, origins.ts, observer.ts converts Y.Doc mutations to TaskStore updates)
