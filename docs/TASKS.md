...
- [ ] Add section on preflight checks (`preflight_check()`)
- [ ] Add section on partial stage success (succeeded/failed group tracking, merge skipping)
- [ ] Add section on stall detection watchdog (`monitor_agent()`, `STALL_TIMEOUT` env var)
- [ ] Add section on model selection (`MODEL` env var → `--model` flag)
- [ ] Add `resume` command to the launch-phase.sh commands list
...
- [ ] Line 34 says "full interactive output (tool calls, diffs, thinking)" — this is wrong
- [ ] Line 74 correctly says "-p mode shows streaming text, not the full rich TUI"
- [ ] Make both descriptions consistent: WATCH mode shows streaming text output, not full TUI
- [ ] Note this is a known limitation — `-p` is required for auto-exit and `--max-budget-usd`

**E3: Add pre-commit hook reference to CLAUDE.md**
- [ ] Add `scripts/pre-commit-hook.sh` to the Development Environment section
- [ ] Include install command: `ln -sf ../../scripts/pre-commit-hook.sh .git/hooks/pre-commit`
...
- [ ] Add specific gotchas section (column name conventions, date serialization format, row ordering)
- [ ] Document the actual data mapping between Ganttlet task fields and Sheets columns
- [ ] Add known failure modes (token expiry during sync, rate limiting, concurrent edits)
- [ ] Reference actual source files: `sheetsClient.ts`, `sheetsMapper.ts`, `sheetsSync.ts`

**F2: Enrich cloud-deployment skill**
- [ ] Add deployment troubleshooting patterns (common Cloud Run failures, health check debugging)
- [ ] Document specific `gcloud` commands used in the deploy pipeline
- [ ] Add the staging → production promotion flow
- [ ] Reference actual files: `deploy.yml`, `Dockerfile.server`, `deploy/frontend/Dockerfile`
- [ ] Add gotchas from deploy workflow (WIF auth, artifact registry paths, env var injection)
