...
export default function TaskBar({
  ...
  owner, functionalArea, showOwner, showArea,
}: TaskBarProps) {
  ...
  const tooltipContent = (
    <div className="space-y-1">
      ...
      <div className="text-gray-400">{done ? 'Done' : 'In progress'}</div>
      {notes && <div className="text-gray-500 text-xs italic">{notes}</div>}
    </div>
  );
