import type { Task, ChangeRecord, FakeUser, ColumnConfig } from '../types';

export const fakeTasks: Task[] = [
  // Root: Q2 Product Launch
  {
    id: 'root',
    name: 'Q2 Product Launch',
    startDate: '2026-03-02',
    endDate: '2026-06-26',
    duration: 84,
    owner: 'Sarah Chen',
    workStream: 'Q2 Product Launch',
    project: 'Q2 Product Launch',
    functionalArea: 'Management',
    percentComplete: 12,
    isMilestone: false,
    isSummary: true,
    parentId: null,
    childIds: ['pe', 'ux', 'gtm', 'ms-launch'],
    dependencies: [],
    isExpanded: true,
    isHidden: false,
    notes: 'Overall Q2 launch initiative',
  },

  // === PLATFORM ENGINEERING ===
  {
    id: 'pe',
    name: 'Platform Engineering',
    startDate: '2026-03-02',
    endDate: '2026-05-29',
    duration: 63,
    owner: 'Marcus Johnson',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 18,
    isMilestone: false,
    isSummary: true,
    parentId: 'root',
    childIds: ['pe-1', 'pe-2', 'pe-3', 'pe-4', 'pe-5', 'pe-6', 'pe-7', 'pe-8', 'pe-9', 'ms-api'],
    dependencies: [],
    isExpanded: true,
    isHidden: false,
    notes: 'Core platform work for Q2',
  },
  {
    id: 'pe-1',
    name: 'API schema design',
    startDate: '2026-03-02',
    endDate: '2026-03-13',
    duration: 10,
    owner: 'Marcus Johnson',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 100,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [],
    isExpanded: false,
    isHidden: false,
    notes: 'Define REST & GraphQL schemas',
  },
  {
    id: 'pe-2',
    name: 'Database migration planning',
    startDate: '2026-03-09',
    endDate: '2026-03-20',
    duration: 10,
    owner: 'Alex Rivera',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 75,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-1', toId: 'pe-2', type: 'SS', lag: 5 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Plan migration from Postgres 14 → 16',
  },
  {
    id: 'pe-3',
    name: 'Auth service refactor',
    startDate: '2026-03-16',
    endDate: '2026-04-03',
    duration: 15,
    owner: 'Marcus Johnson',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 40,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-1', toId: 'pe-3', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Move to OAuth2 + PKCE flow',
  },
  {
    id: 'pe-4',
    name: 'Execute database migration',
    startDate: '2026-03-23',
    endDate: '2026-04-10',
    duration: 15,
    owner: 'Alex Rivera',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'DevOps',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-2', toId: 'pe-4', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Execute staged migration with rollback plan',
  },
  {
    id: 'pe-5',
    name: 'API endpoint implementation',
    startDate: '2026-04-06',
    endDate: '2026-04-24',
    duration: 15,
    owner: 'Marcus Johnson',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-3', toId: 'pe-5', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Build all CRUD endpoints',
  },
  {
    id: 'pe-6',
    name: 'Load testing & optimization',
    startDate: '2026-04-27',
    endDate: '2026-05-08',
    duration: 10,
    owner: 'Jordan Kim',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'QA',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [
      { fromId: 'pe-5', toId: 'pe-6', type: 'FS', lag: 2 },
      { fromId: 'pe-4', toId: 'pe-6', type: 'FS', lag: 0 },
    ],
    isExpanded: false,
    isHidden: false,
    notes: 'Target: 10k RPS, p99 < 200ms',
  },
  {
    id: 'pe-7',
    name: 'CI/CD pipeline updates',
    startDate: '2026-04-13',
    endDate: '2026-04-24',
    duration: 10,
    owner: 'Alex Rivera',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'DevOps',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-4', toId: 'pe-7', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Update deploy scripts for new infra',
  },
  {
    id: 'pe-8',
    name: 'API documentation',
    startDate: '2026-04-27',
    endDate: '2026-05-08',
    duration: 10,
    owner: 'Priya Patel',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-5', toId: 'pe-8', type: 'FF', lag: 0 }],
    isExpanded: false,
    isHidden: false,
    notes: 'OpenAPI spec + developer portal',
  },
  {
    id: 'pe-9',
    name: 'Security audit',
    startDate: '2026-05-11',
    endDate: '2026-05-29',
    duration: 15,
    owner: 'Jordan Kim',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'QA',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-6', toId: 'pe-9', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'OWASP top-10 + pen testing',
  },
  {
    id: 'ms-api',
    name: 'API Ready',
    startDate: '2026-05-29',
    endDate: '2026-05-29',
    duration: 0,
    owner: 'Marcus Johnson',
    workStream: 'Platform Engineering',
    project: 'API Overhaul',
    functionalArea: 'Engineering',
    percentComplete: 0,
    isMilestone: true,
    isSummary: false,
    parentId: 'pe',
    childIds: [],
    dependencies: [{ fromId: 'pe-9', toId: 'ms-api', type: 'FS', lag: 0 }],
    isExpanded: false,
    isHidden: false,
    notes: 'API feature-complete milestone',
  },

  // === USER EXPERIENCE ===
  {
    id: 'ux',
    name: 'User Experience',
    startDate: '2026-03-09',
    endDate: '2026-06-05',
    duration: 63,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 15,
    isMilestone: false,
    isSummary: true,
    parentId: 'root',
    childIds: ['ux-1', 'ux-2', 'ux-3', 'ux-4', 'ux-5', 'ux-6', 'ux-7', 'ux-8', 'ms-ux'],
    dependencies: [],
    isExpanded: true,
    isHidden: false,
    notes: 'Design & frontend for Q2',
  },
  {
    id: 'ux-1',
    name: 'User research & interviews',
    startDate: '2026-03-09',
    endDate: '2026-03-20',
    duration: 10,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 100,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [],
    isExpanded: false,
    isHidden: false,
    notes: '12 user interviews scheduled',
  },
  {
    id: 'ux-2',
    name: 'Wireframes & prototyping',
    startDate: '2026-03-23',
    endDate: '2026-04-03',
    duration: 10,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 30,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-1', toId: 'ux-2', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Figma prototypes for 5 key flows',
  },
  {
    id: 'ux-3',
    name: 'Design system components',
    startDate: '2026-04-06',
    endDate: '2026-04-24',
    duration: 15,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-2', toId: 'ux-3', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Build reusable React component library',
  },
  {
    id: 'ux-4',
    name: 'Frontend integration',
    startDate: '2026-04-27',
    endDate: '2026-05-15',
    duration: 15,
    owner: 'Sarah Chen',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Engineering',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [
      { fromId: 'ux-3', toId: 'ux-4', type: 'FS', lag: 2 },
      { fromId: 'pe-5', toId: 'ux-4', type: 'SF', lag: 0 },
    ],
    isExpanded: false,
    isHidden: false,
    notes: 'Wire up frontend to new API',
  },
  {
    id: 'ux-5',
    name: 'Accessibility audit',
    startDate: '2026-04-20',
    endDate: '2026-05-01',
    duration: 10,
    owner: 'Jordan Kim',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'QA',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-3', toId: 'ux-5', type: 'SS', lag: 10 }],
    isExpanded: false,
    isHidden: false,
    notes: 'WCAG 2.1 AA compliance check',
  },
  {
    id: 'ux-6',
    name: 'Usability testing',
    startDate: '2026-05-04',
    endDate: '2026-05-15',
    duration: 10,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-4', toId: 'ux-6', type: 'SS', lag: 5 }],
    isExpanded: false,
    isHidden: false,
    notes: '8 participants, think-aloud protocol',
  },
  {
    id: 'ux-7',
    name: 'Design iteration & polish',
    startDate: '2026-05-18',
    endDate: '2026-05-29',
    duration: 10,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-6', toId: 'ux-7', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Incorporate usability feedback',
  },
  {
    id: 'ux-8',
    name: 'E2E testing',
    startDate: '2026-05-25',
    endDate: '2026-06-05',
    duration: 10,
    owner: 'Jordan Kim',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'QA',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-7', toId: 'ux-8', type: 'SS', lag: 5 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Playwright test suite',
  },
  {
    id: 'ms-ux',
    name: 'UX Complete',
    startDate: '2026-06-05',
    endDate: '2026-06-05',
    duration: 0,
    owner: 'Priya Patel',
    workStream: 'User Experience',
    project: 'Design System',
    functionalArea: 'Design',
    percentComplete: 0,
    isMilestone: true,
    isSummary: false,
    parentId: 'ux',
    childIds: [],
    dependencies: [{ fromId: 'ux-8', toId: 'ms-ux', type: 'FS', lag: 0 }],
    isExpanded: false,
    isHidden: false,
    notes: 'UX sign-off milestone',
  },

  // === GO-TO-MARKET ===
  {
    id: 'gtm',
    name: 'Go-to-Market',
    startDate: '2026-03-16',
    endDate: '2026-06-19',
    duration: 68,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 10,
    isMilestone: false,
    isSummary: true,
    parentId: 'root',
    childIds: ['gtm-1', 'gtm-2', 'gtm-3', 'gtm-4', 'gtm-5', 'gtm-6', 'gtm-7', 'ms-gtm'],
    dependencies: [],
    isExpanded: true,
    isHidden: false,
    notes: 'Marketing & launch prep',
  },
  {
    id: 'gtm-1',
    name: 'Competitive analysis',
    startDate: '2026-03-16',
    endDate: '2026-03-27',
    duration: 10,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 60,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [],
    isExpanded: false,
    isHidden: false,
    notes: 'Analyze top 5 competitors',
  },
  {
    id: 'gtm-2',
    name: 'Messaging & positioning',
    startDate: '2026-03-30',
    endDate: '2026-04-10',
    duration: 10,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [{ fromId: 'gtm-1', toId: 'gtm-2', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Core value props & differentiators',
  },
  {
    id: 'gtm-3',
    name: 'Website redesign',
    startDate: '2026-04-13',
    endDate: '2026-05-01',
    duration: 15,
    owner: 'Priya Patel',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Design',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [{ fromId: 'gtm-2', toId: 'gtm-3', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Landing page + docs site refresh',
  },
  {
    id: 'gtm-4',
    name: 'Content creation',
    startDate: '2026-04-20',
    endDate: '2026-05-08',
    duration: 15,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [{ fromId: 'gtm-2', toId: 'gtm-4', type: 'FS', lag: 7 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Blog posts, case studies, demo videos',
  },
  {
    id: 'gtm-5',
    name: 'Beta program setup',
    startDate: '2026-05-11',
    endDate: '2026-05-22',
    duration: 10,
    owner: 'Alex Rivera',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Product',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [
      { fromId: 'gtm-4', toId: 'gtm-5', type: 'FS', lag: 2 },
      { fromId: 'ms-api', toId: 'gtm-5', type: 'FS', lag: -10 },
    ],
    isExpanded: false,
    isHidden: false,
    notes: 'Onboard 20 beta customers',
  },
  {
    id: 'gtm-6',
    name: 'Launch event planning',
    startDate: '2026-05-25',
    endDate: '2026-06-12',
    duration: 15,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [{ fromId: 'gtm-5', toId: 'gtm-6', type: 'FS', lag: 2 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Virtual launch event + webinar series',
  },
  {
    id: 'gtm-7',
    name: 'Sales enablement',
    startDate: '2026-06-01',
    endDate: '2026-06-19',
    duration: 15,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 0,
    isMilestone: false,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [{ fromId: 'gtm-6', toId: 'gtm-7', type: 'SS', lag: 5 }],
    isExpanded: false,
    isHidden: false,
    notes: 'Training decks, battle cards, demo scripts',
  },
  {
    id: 'ms-gtm',
    name: 'Launch Ready',
    startDate: '2026-06-19',
    endDate: '2026-06-19',
    duration: 0,
    owner: 'Sarah Chen',
    workStream: 'Go-to-Market',
    project: 'Marketing Push',
    functionalArea: 'Marketing',
    percentComplete: 0,
    isMilestone: true,
    isSummary: false,
    parentId: 'gtm',
    childIds: [],
    dependencies: [
      { fromId: 'gtm-7', toId: 'ms-gtm', type: 'FS', lag: 0 },
      { fromId: 'ms-ux', toId: 'ms-gtm', type: 'FS', lag: 0 },
    ],
    isExpanded: false,
    isHidden: false,
    notes: 'All systems go for launch',
  },

  // Final milestone
  {
    id: 'ms-launch',
    name: 'Product Launch',
    startDate: '2026-06-26',
    endDate: '2026-06-26',
    duration: 0,
    owner: 'Sarah Chen',
    workStream: 'Q2 Product Launch',
    project: 'Q2 Product Launch',
    functionalArea: 'Management',
    percentComplete: 0,
    isMilestone: true,
    isSummary: false,
    parentId: 'root',
    childIds: [],
    dependencies: [
      { fromId: 'ms-api', toId: 'ms-launch', type: 'FS', lag: 0 },
      { fromId: 'ms-gtm', toId: 'ms-launch', type: 'FS', lag: 5 },
    ],
    isExpanded: false,
    isHidden: false,
    notes: 'Ship it!',
  },
];

export const fakeUsers: FakeUser[] = [
  { id: 'u1', name: 'Sarah Chen', avatar: 'SC', color: '#3b82f6', isOnline: true, viewingTaskId: 'pe-3' },
  { id: 'u2', name: 'Marcus Johnson', avatar: 'MJ', color: '#22c55e', isOnline: true, viewingTaskId: null },
  { id: 'u3', name: 'Priya Patel', avatar: 'PP', color: '#a855f7', isOnline: false, viewingTaskId: null },
];

export const fakeChangeHistory: ChangeRecord[] = [
  { id: 'ch-1', timestamp: '2026-03-01T14:23:00Z', user: 'Sarah Chen', taskId: 'pe-1', taskName: 'API schema design', field: 'percentComplete', oldValue: '80', newValue: '100' },
  { id: 'ch-2', timestamp: '2026-03-01T13:45:00Z', user: 'Marcus Johnson', taskId: 'pe-3', taskName: 'Auth service refactor', field: 'endDate', oldValue: '2026-03-30', newValue: '2026-04-03' },
  { id: 'ch-3', timestamp: '2026-03-01T11:12:00Z', user: 'Priya Patel', taskId: 'ux-2', taskName: 'Wireframes & prototyping', field: 'percentComplete', oldValue: '10', newValue: '30' },
  { id: 'ch-4', timestamp: '2026-02-28T16:30:00Z', user: 'Alex Rivera', taskId: 'pe-2', taskName: 'Database migration planning', field: 'owner', oldValue: 'Marcus Johnson', newValue: 'Alex Rivera' },
  { id: 'ch-5', timestamp: '2026-02-28T15:00:00Z', user: 'Sarah Chen', taskId: 'gtm-1', taskName: 'Competitive analysis', field: 'percentComplete', oldValue: '40', newValue: '60' },
  { id: 'ch-6', timestamp: '2026-02-28T10:20:00Z', user: 'Jordan Kim', taskId: 'pe-6', taskName: 'Load testing & optimization', field: 'duration', oldValue: '8', newValue: '10' },
  { id: 'ch-7', timestamp: '2026-02-27T14:50:00Z', user: 'Marcus Johnson', taskId: 'pe-5', taskName: 'API endpoint implementation', field: 'startDate', oldValue: '2026-04-01', newValue: '2026-04-06' },
  { id: 'ch-8', timestamp: '2026-02-27T09:30:00Z', user: 'Priya Patel', taskId: 'ux-1', taskName: 'User research & interviews', field: 'percentComplete', oldValue: '85', newValue: '100' },
  { id: 'ch-9', timestamp: '2026-02-26T16:15:00Z', user: 'Sarah Chen', taskId: 'root', taskName: 'Q2 Product Launch', field: 'name', oldValue: 'Q2 Release', newValue: 'Q2 Product Launch' },
  { id: 'ch-10', timestamp: '2026-02-26T11:00:00Z', user: 'Alex Rivera', taskId: 'pe-7', taskName: 'CI/CD pipeline updates', field: 'notes', oldValue: 'Update CI', newValue: 'Update deploy scripts for new infra' },
  { id: 'ch-11', timestamp: '2026-02-25T15:45:00Z', user: 'Marcus Johnson', taskId: 'pe-2', taskName: 'Database migration planning', field: 'percentComplete', oldValue: '50', newValue: '75' },
  { id: 'ch-12', timestamp: '2026-02-25T10:30:00Z', user: 'Priya Patel', taskId: 'gtm-3', taskName: 'Website redesign', field: 'duration', oldValue: '10', newValue: '15' },
];

export const defaultColumns: ColumnConfig[] = [
  { key: 'name', label: 'Task Name', width: 240, visible: true },
  { key: 'owner', label: 'Owner', width: 120, visible: true },
  { key: 'startDate', label: 'Start', width: 90, visible: true },
  { key: 'endDate', label: 'End', width: 90, visible: true },
  { key: 'duration', label: 'Duration', width: 70, visible: true },
  { key: 'percentComplete', label: '% Done', width: 65, visible: true },
  { key: 'functionalArea', label: 'Area', width: 100, visible: false },
  { key: 'workStream', label: 'Work Stream', width: 140, visible: false },
  { key: 'project', label: 'Project', width: 120, visible: false },
];
